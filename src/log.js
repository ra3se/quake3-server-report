...
const events = {
    init: {
        regExp: "InitGame: \\\\(.+)",
        parser: ([data]) => {
            return Object.assign(parseDataString(data), {date});
        },
    },
    shutdown: {
        regExp: "ShutdownGame:",
    ...
    kill: {
        ...
        parser: ([n1, n2, n3, n4, killer, victim, weapon]) => 
            ({killer, victim, weapon}),
    },
    killed: {
        regExp: `Kill: (\\d+) (\\d+) (\\d+) (\\d+): <world> killed ([^\\s]+) by ${worldDamageRegExp}`,
        parser: ([n1, n2, n3, n4, victim, damage]) => ({victim, damage}),
    },
    say: {
        regExp: "say: (\\d+) (\\d+): ([^:]+): (.+)",
        parser: ([n1, n2, player, message]) => 
            ({player, message}),
    },
};
