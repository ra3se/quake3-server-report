...
const serverEventParser = require('../../src/lib/server-event-parser');

o('unknown', () => {
	const {event, data} = serverEventParser('Some random line');
...
o('remove syslog', () => {
	const {event, data} = serverEventParser('<27>May 25 23:28:26 fda94d26f1ca[586]: Some random line');
...
o('shutdown', () => {
	const {event, data} = serverEventParser('ShutdownGame:');
...
o('connect', () => {
	const {event, data} = serverEventParser('ClientConnect: 0 ^0k^3h4^0rma^3! (127.0.0.1)');
...
o('disconnect', () => {
	const {event, data} = serverEventParser('ClientDisconnect: 0');
...
o('info', () => {
	const {event, data} = serverEventParser('ClientUserinfoChanged: 0 n\\InsiPopinsi\\t\\5954957\\rt\\5954957\\model\\doom/default\\c1\\4\\c2\\5\\hc\\100\\w\\0\\l\\0\\a\\0\\ec\\1051');
...
o.spec('kill', () => {
	o('Player kill', () => {
		const {event, data} = serverEventParser('Kill: 1 0 10 4: Shizu killed ^1K^7orroz by MOD_RAILGUN');
	...
	o('World death', () => {
		const {event, data} = serverEventParser('Kill: 1022 0 19 2: <world> killed ^1K^7orroz by MOD_FALLING');
...
o('message', () => {
	const {event, data} = serverEventParser('say: 0 2: Ketch: nostalgi');
